...
const ProfileCardComponent = ({
  avatarUrl = 'https://images.unsplash.com/photo-1529665253569-6d01c0eaf7b6?auto=format&fit=crop&w=600&q=80',
  backgroundUrl = interviewBg,
  iconUrl,
  grainUrl,
  innerGradient,
  ...
  const cardStyle = useMemo(
    () => ({
      ...
      '--ui-accent': accentColor ?? 'rgba(255, 255, 255, 0.1)',
      '--ui-accent-border': accentBorder ?? 'rgba(255, 255, 255, 0.12)'
    }),
    [iconUrl, grainUrl, innerGradient, behindGlowColor, behindGlowSize, accentColor, accentBorder]
  );
  ...
  return (
    <div className="terminal-page">
      ...
      <div className="panel interviews-panel">
        <header className="interviews-head">
          ...
          {!loading &&
            interviews.map((interview, idx) => (
              <ProfileCard
                ...
                title="Suspect"
                handle={toHandle(interview.name)}
                status={
                  interview.guilt_level == null || interview.guilt_level === -1
                    ? 'Guilt: Unknown'
                    : `Guilt: ${interview.guilt_level}`
                }
                backgroundUrl={interview.background_url}
                contactText="Open"
                behindGlowEnabled={false}
                className="pc-compact"
                accentColor={guiltToAccent(interview.guilt_level).bg}
                accentBorder={guiltToAccent(interview.guilt_level).border}
                onContactClick={() => handleContactClick(interview.name)}
              />
            ))}
        </div>
      </div>
